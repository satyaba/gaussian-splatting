...
def main():
    ...
    # NOTE: optimizer state is legitimately EMPTY in the L_seg-inactive state
    # (Task 12 deferred): the decoder receives no gradient until gt_segmentation
    ...
    print(f"   decoder50 OK: iteration={dec50_state['iteration']}, "
          ...
          f"optimizer_state {'EMPTY (L_seg inactive — expected)' if dec_state_empty_at_50 else 'present'}")
    ...
    # seg encoding only moves when L_seg is active (Task 12 deferred); in the
    ...
    if dec_state_empty_at_50:
        assert seg_moved == 0.0, "seg encoding changed while L_seg inactive — unexpected drift"
    else:
        assert seg_moved > 0.0, "seg encoding did not change after resume (L_seg active)"
    ...
    if not dec_state_empty_at_50:
        ...
        assert w_moved == 0.0, "decoder weights changed while L_seg inactive — unexpected drift"
