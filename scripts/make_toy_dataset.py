...
#   out/train_sem/r_XX.png  (uint8 class ids: 0=wall,1=red blob,2=green blob, 255=void/ignore)
#   out/train_depth/r_XX.png (float32 inverse depth, 0 = invalid)
#
# NOTE: the semantic/depth folders are NOT wired into the loader yet (Task 12
...
def render_frame(azimuth, height, size, seed=0):
    ...
    sem = np.full((size, size), 0, dtype=np.uint8)                  # class 0
...
def main():
    ...
    for sub in ("train", "test", "train_sem", "train_depth"):
    ...
    for i in range(args.frames):
        ...
        img, sem, dep = render_frame(az, h, args.size, seed=i)
        stem = os.path.join(args.out, name)
        Image.fromarray((img * 255).astype(np.uint8)).save(stem + ".png")
        cv2.imwrite(os.path.join(args.out, "train_sem", f"r_{i:02d}.png"), sem)
