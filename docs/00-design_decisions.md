...
Loss terms: L_seg (cross-entropy), L_consist (multi-view consistency, doubles as confidence signal for the 0.75 splitting threshold), L_edge (boundary sharpness), L_parsimony (segmentation-gated densification restraint), L_photo (dual-role: low-weight stabilizer in Stage 1, dominant in Stage 2), L_lineage (discounted identity preservation, Stage 2 only).
...
Segmentation must drive geometry gradients: dL_dalpha must accumulate from both color and segmentation losses; silently dropping the segmentation contribution would undermine the central thesis claim that L_seg actively drives geometry
